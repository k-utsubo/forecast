...
class DataMaker(object):
    ...
    def make(self):
        ...
        # select i.cprice,u.price,e.price,d.cl,i.date from indexHist i,otherHist u,otherHist e,idcStockDaily d where i.date=u.date and i.date=e.date and i.date=d.date and i.indexCode="101" and u.otherCode="FEXCH" and e.otherCode="EURO" and d.indexCode="I_DJI"
        result = cursor.fetchall()
        cursor.close()
        return xp.asarray( result, np.float32)
    ...
    def make_mini_batch(self, data, mini_batch_size, length_of_sequence):
        sequences = xp.ndarray(( mini_batch_size, length_of_sequence,IN_UNITS), dtype=np.float32)
...
if __name__ == "__main__":
    ...
    # save model
    cPickle.dump(model, open("./23_model.bin", "wb"))
