...
IN_UNITS = 1  # nk,dax,djの順で一つの時系列にしてみる
OUT_UNITS = 1
DISPLAY_EPOCH = 100
...
class DataMaker(object):
    ...
    def make(self,xp,train):
        ...
        cursor.execute("select i.cprice,x.cl,d.cl from indexHist i,idcStockDaily d,idcStockDaily x where i.date=x.date and  i.date=d.date and i.indexCode='101' and d.indexCode='I_DJI' and x.indexCode='DAX' and i.date>=%s and i.date<=%s order by i.date asc",[fromDate,toDate])

        result = cursor.fetchall()
        cursor.close()
        items=xp.asarray(result, np.float32)
        ret=((items[1:]-items[:-1])/items[:-1])
        flat=ret.flatten()
        return flat
...
def training_main(xp, in_units, hidden_units, out_units,training_epochs,mini_batch_size,length_of_sequence):
    model_path="./38_model_"+str(hidden_units)+"_"+str(training_epochs)+"_"+str(length_of_sequence)+".bin"
    # make training data
...
def predict_main(xp, model_path):
    ...
    for i in range(0,len(data)-1):
        ...
        if i % 3 == 0:
            ...
            real_nk225=real_nk225*(1+data[i+1])
            print(str(i)+",predict price="+str(predict_nk225)+",real price="+str(real_nk225))+",predict ratio="+str(pprice[0][0])+",real ratio="+str(data[i+1])
        now_price=pprice[0][0]


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--gpu', '-g', default=-1, type=int, help='GPU ID (negative value indicates CPU)')
    parser.add_argument('--hidden','-d',default=20, type=int)
    parser.add_argument('--epochs','-e',default=20000, type=int)
    parser.add_argument('--size','-s',default=50, type=int)
