...
IN_UNITS = 8  # nk,dj,dax,usdjpy,eurjpy,gold,oil,bond
OUT_UNITS = 8
DISPLAY_EPOCH = 100
...
class DataMaker(object):
    ...
    def make(self,xp,train):
        ...
        cursor.execute("select i.cprice,d.cl,x.cl,e.price,g.price,u.price,o.price,b.price from indexHist i,idcStockDaily d,idcStockDaily x,otherHist e,otherHist g,otherHist u ,otherHist o,otherHist b where i.date=x.date and  i.date=d.date and i.date=g.date and i.date=e.date and i.date=u.date and i.date=o.date and i.date=b.date and i.indexCode='101' and d.indexCode='I_DJI' and x.indexCode='DAX' and e.otherCode='EURO' and g.otherCode='GOLD' and u.otherCode='FEXCH' and o.otherCode='OIL' and b.otherCode='LBOND' and i.date>=%s and i.date<=%s order by i.date asc",[fromDate,toDate])
...
def training_main(xp, in_units, hidden_units, out_units,training_epochs,mini_batch_size,length_of_sequence):
    model_path="./36_model_"+str(hidden_units)+"_"+str(training_epochs)+"_"+str(length_of_sequence)+".bin"
    # make training data
