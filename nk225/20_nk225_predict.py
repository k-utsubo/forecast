...
class DataMaker(object):
    ...
    def make(self):
        ...
        cursor.close()
        return np.asarray(map(lambda x:x[0],result),np.float32)
...
if __name__ == "__main__":
    ...
    for i in range(1,len(data)-1):
        now_price=data[i]
        next_price=data[i+1]
        predict_price=predict(model,now_price)
        print(str(i)+",predict price="+str(predict_price[0][0])+",real price="+str(next_price))
        predict_prices.append(predict_price[0][0])
