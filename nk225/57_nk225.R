...
date_list<-function(fmDate,toDate){
  query<-paste("select date_format(i.date,'%Y/%m/%d') as date from ST_priceHistAdj i,idcStockDaily d,idcStockDaily x where i.date=x.date and  i.date=d.date  and i.stockCode='",stockCode,"' and d.indexCode='I_DJI' and x.indexCode='DAX'  and i.date>='",fmDate,"' and i.date<='",toDate,"' order by i.date asc",sep="")
  data<-fetch(dbSendQuery(con,query),n=-1)
  return(as.character(data$date))
}
calc_forecast_open<-function(date){
  query<-paste("select i.cprice as nk225,i.oprice as nk225o,d.cl as dj,x.cl as dax from ST_priceHistAdj i,idcStockDaily d,idcStockDaily x where i.date=x.date and  i.date=d.date and i.stockCode='",stockCode,"' and d.indexCode='I_DJI' and x.indexCode='DAX' and i.date>=date_add('",date,"',interval -1 year) and i.date<'",date,"' order by i.date asc",sep="")
  data<-fetch(dbSendQuery(con,query),n=-1)
  ...
  query<-paste("select i.cprice as nk225,i.oprice as nk225o,d.cl as dj,x.cl as dax from ST_priceHistAdj i,idcStockDaily d,idcStockDaily x where i.date=x.date and  i.date=d.date and i.stockCode='",stockCode,"' and d.indexCode='I_DJI' and x.indexCode='DAX' and i.date<='",date,"' order by i.date desc limit 3",sep="")
  data<-fetch(dbSendQuery(con,query),n=-1)
  ...
  return(data.frame(predict=data.predict,real=data.real,diff=data.diff,price=data.price,priceupp=data.priceupp,pricelow=data.pricelow,pricenext=data.pricenext))
}
...
data.diff<-c()
data.date<-c()
data.df=data.frame(data=data.date,predict=data.predict,real=data.real,diff=data.diff,price=data.price,priceupp=data.priceupp,pricelow=data.pricelow,pricenext=data.pricenext)

date.list<-date_list("2006/1/1","2016/10/12")
#date.list<-date_list("2013/4/12","2016/10/12")
...
data.open<-read.table("57_open.txt")
data.open<-data.open[,colnames(data.open)!="V1"]
names(data.open)<-c("yosoku","seikai","diff","price","priceupp","pricelow","priceseikai","date")

## ETC 1321
data.etf<-fetch(dbSendQuery(con,paste("select date_format(date,'%Y/%m/%d') as date ,oprice,high,low,cprice  from ST_priceHistAdj where stockCode='",stockCode,"' and date>='",date.list[1],"' and date<='",date.list[length(date.list)],"' order by date",sep="")),n=-1)
data.etf$date<-as.POSIXct(data.etf$date)
data.ok<-merge(data.open,data.etf,by="date")
...
par(new=T)
plot(data.etf$date,data.etf$cprice/data.etf[1,]$cprice,col="red",type="l",ylim=c(0,4),xlim=c(as.POSIXct("2006-01-05"),as.POSIXct("2016-10-12")),xlab="date",ylab="ratio")
legend("topleft",legend=c("simu",stockCode),col=c("black","red"),lty=c(1,1))
