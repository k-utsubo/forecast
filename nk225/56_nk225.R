...
data.open<-read.table("56_open.txt")
...
## ETC 1321
data.etf<-fetch(dbSendQuery(con,paste("select date_format(date,'%Y/%m/%d') as date ,oprice,high,low,cprice  from ST_priceHistAdj where stockCode='",stockCode,"' order by date",sep="")),n=-1)
