...
class DataMaker(object):
    ...
    def make(self,xp,train):
        ...
        cursor.execute("select i.cprice from indexHistMonthly i where i.indexCode='101' and i.date>=%s and i.date<=%s order by i.date asc",[fromDate,toDate])
...
def training_main(xp, in_units, hidden_units, out_units,training_epochs,mini_batch_size,length_of_sequence):
    model_path="./11_model_"+str(hidden_units)+"_"+str(training_epochs)+"_"+str(length_of_sequence)+".bin"
    # make training data
