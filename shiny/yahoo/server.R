...
getkeywords<-function(session){
    ...
    sql<-paste("select * from fintech.yahooStockNewsKeyword where href='http://headlines.yahoo.co.jp/hl?a=",query$url,"'",sep="")
...
getnews<-function(session){
    ...
    con<-dbConnect(dbDriver("MySQL"),dbname="fintech",user="root",password="",host="zaaa16d")
    res<-dbGetQuery(con, "set names utf8") 
    sql<-paste("select date,title,body from fintech.yahooStockNews where href='http://headlines.yahoo.co.jp/hl?a=",query$url,"'",sep="")
    res<-dbSendQuery(con,sql)
    data.df<-fetch(res,-1)
    dbDisconnect(con)
    return(data.df)
...
shinyServer(function(input, output,session) {
  output$newsBody <- renderDataTable({
    ...
    b<-modifybody(data.df$body,data.kw)
    data.frame(body=c(b))
  }, escape = FALSE)
})
