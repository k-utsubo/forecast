...
library(hash)
library(RSQLite)
setwd("/Volumes/ubuntu/sentiment/senti/topic3")
set.seed(123)

con<-dbConnect(dbDriver("SQLite"),"../../../data/news_topic3.db")
res<-dbSendQuery(con," select doc,word,count from news_bow where word in (select word from news_idf where idf<4)")
data.bow<-fetch(res,n=-1)
