      ...
      open(file) do |io|
        ...
        if mc.size==1 then
          ...
          if score_ary.length>0 then
            ...
            stmt=mysql.prepare("replace into fintech.yahooStockNewsSentiment2 values(?,?,?,?,?,?,?,?)")
...
p json
            ...
            mysql=get_mysql_fintech()
            stmt=mysql.prepare("replace into fintech.yahooStockNewsPrice2 values(?,?,?,?,?,?,?,?,?,?,?,?,?)")
  ...
  def wakati_analyze(body,score,href)
    ...
    keys.each do |keyitem|
      ...
      if @dict.has_key?(key) and @dict[key][0].to_f!=0  then
        ...
        mysql=get_mysql_fintech()
        stmt=mysql.prepare("replace into fintech.yahooStockNewsKeyword2 values(?,?,?,?,?,?,?,?,?,?,?)")
